...
from basicsr.data.data_util import (paired_paths_from_folder,
                                    dehaze_paired_paths_from_folder,
...
class Dehaze_Dataset(data.Dataset):
    """Dehaze Dataset for image restoration.
    ...
    def __init__(self, opt):
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            #     [self.lq_folder, self.gt_folder], ['lq', 'gt'],
            #     self.filename_tmpl)
            self.paths = dehaze_paired_paths_from_folder(
                [self.lq_folder, self.gt_folder], ['lq', 'gt'],
                self.filename_tmpl)
    ...
    def __getitem__(self, index):
        ...
        # print(lq_path)
        # gt_path = self.paths[index]['lq_path']
        # if self.opt['phase'] == 'train':
        #     gt_path = lq_path.replace("hazy", "clear").split('_')[0] + '.jpg'
        # else:
        gt_path = lq_path.replace("hazy", "gt").split('_')[0] + '.png'
